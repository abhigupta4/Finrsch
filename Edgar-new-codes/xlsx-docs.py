...
def download_excel(link1, cik, date):
	path = os.path.join('Excel',cik)
	...
	with open(path + '/' + date + '.' + link1.split('/')[-1].split('.')[-1], 'wb') as f:
		f.write(r.content)
		f.close()
...
def take_second_link(cur,cik):
	...
	for link in links:
		temp = link.get("href")
		if '.xlsx' in temp or '.xls' in temp:
			sec = begin + temp
			break
		...
		try:
			for i in range(len(temp)):
				if temp[i] == '10-Q' or temp[i] == '10-K' and temp[i-1] == '' and temp[i+1] == '':
					...
					if financial != '':
						download_excel(financial, name, filing)
						if name in testing:
							testing[name] += 1
						else:
							testing[name] = 1

						f1 = open("account1.txt","w")
						f1.write("year = " + str(year) + "\n")
						f1.write("quarter = " + str(quarter) + "\n")
						f1.write(str(testing) + '\n')
						f1.close()
					break
		except:
			pass
